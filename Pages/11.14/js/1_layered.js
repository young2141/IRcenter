...
function parsing1(year, condition, data) {
    ...
    $.getJSON("../../json/" + filename, (jsonData) => {
        temp["recruitment"] = jsonData[condition]["recruitment_" + condition + "_admission"];
        temp["applied"] = jsonData[condition]["applied_" + condition + "_admission"];
        temp["admitted"] = jsonData[condition]["admitted_" + condition + "_admission"];
        data.push(temp);
        if (data.length === 10) {
