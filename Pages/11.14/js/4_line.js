...
function drawLineChart(_data1, _data2, _condition1, _condition2) {
    am4core.ready(function () {
        ...
        for (var i = 1; i < 10; i++) {
            if (max1 < _data1[i]["competition_rate"]) max1 = _data1[i]["competition_rate"]
            if (max2 < _data2[i]["recruitment_rate"]) max2 = _data2[i]["recruitment_rate"]
            if (min1 > _data1[i]["competition_rate"]) min1 = _data1[i]["competition_rate"]
            if (min2 > _data2[i]["recruitment_rate"]) min2 = _data2[i]["recruitment_rate"]
        }

        createAxisAndSeries(chart1, "competition_rate", "경쟁률", "#00a686", "경쟁", _condition1, _condition2, max1, min1);
        createAxisAndSeries(chart2, "recruitment_rate", "충원율", "#e8cd05", "충원", _condition1, _condition2, max2, min2);
...
function createAxisAndSeries(_chart, _value, _ko, _rgb, _title, _condition1, _condition2, _max, _min) {
    ...
    bullet.tooltipText += "신입생 " + _ko + "은 [bold]{valueY}" + (_ko == "경쟁률" ? ":1" : "%") + "[\] 입니다.";
}