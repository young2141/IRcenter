function parsing1() {
    ...
    while (cnt <= 3) {
		...
		var full_highschool = ["일반고등학교", "특수목적고등학교", "특성화고등학교", "자율고등학교", "기타"];
...
function createSeries1(_chart, _year) {
    ...
    series.columns.template.tooltipText = "[#000]" + String(_year) + "년도 {categoryX} 신입생은 [bold]{valueY}명[\] 입니다.";
    series.columns.template.fillOpacity = .8;
    series.columns.template.adapter.add("fill", function (fill, target) {
        if (target.dataItem.categoryX == "일반고") return am4core.color("#ff3d00");
        else if (target.dataItem.categoryX == "특수목적고") return am4core.color("#ef5350");
        else if (target.dataItem.categoryX == "특성화고") return am4core.color("#ff7f27");
        else if (target.dataItem.categoryX == "자율고등학교") return am4core.color("#ffc102");
        else if (target.dataItem.categoryX == "기타") return am4core.color("#fdf5bc");
    });
