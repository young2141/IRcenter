function iter1(temp, data, start, end, select, callback) {
    for (var i = start; i <= end; i++) { // start년 부터 end년까지! (2010 ~ 2018)
        /**
         * data : 배열들 담을 오브젝트
         * i : 현재 년도
         * start : 뺄 값...minus 값
         */
        doParsing1(data, i, start, select, (year, arrApplied, arrRecruitment, arrAdmitted) => {
            temp.push({// chart.data에 i년도의 값들을 집어넣어라!
                year: String(year),
            ...
            if (temp.length === end - start + 1) {
                temp.sort((a, b) => { return a.year < b.year ? -1 : a.year > b.year ? 1 : 0 });
                callback();
            }
        });
    }
    ...
    $.getJSON("../json/" + file, (jsonData) => {
        ...
        while (len <= jsonData.length) {
            if (len === jsonData.length) {
                callback(year, data.arrApplied[year - minus], data.arrRecruitment[year - minus], data.arrAdmitted[year - minus]);
                break;
            }
            /**
             * select == 1 > 전체
            ...
            if (select === 1) {
                ...
                data.arrRecruitment[year - minus] += jsonData[len].recruitment_within_admission;
                data.arrAdmitted[year - minus] += jsonData[len].man_admitted_within_admission + jsonData[len].woman_admitted_within_admission;
            } else {
                data.arrApplied[year - minus] += jsonData[len].applied_over_admission;
                data.arrRecruitment[year - minus] += jsonData[len].recruitment_over_admission;
                data.arrAdmitted[year - minus] += jsonData[len].man_admitted_over_admission + jsonData[len].woman_admitted_over_admission;
            }
            len++;
        }
    })
...
function drawLayeredChart(arr) {
    var startYear = 2010, endYear = 2019;
    ...
    var temp = [];
    for (var i = 0; i <= endYear - startYear; i++) { // 10개년
    ...
    // Themes begin
    // Themes end
    iter1(temp, dataObject, startYear, endYear, select, () => { // JSON 파일 파싱 시작!
        ...
        var series1 = createSeries1(chart, "total_applied", "총 지원자 수", 80, tooltipApplied);
        series1.columns.template.stroke = am4core.color("#0000ff");
        series1.columns.template.fill = am4core.color("#0000ff");
        var series2 = createSeries1(chart, "total_recruitment", "총 모집인원", 60, tooltipRecruitment);
        series2.columns.template.stroke = am4core.color("#ffff00");
        series2.columns.template.fill = am4core.color("#ffff00");
        var series3 = createSeries1(chart, "total_admitted", "총 입학한 수", 40, tooltipAdmitted);
...
function iter2(temp1, temp2, data, start, end, select, callback) {
    for (var i = start; i <= end; i++) { // start년 부터 end년까지! (2010 ~ 2018)
         ...
         * start : 뺄 값...minus 값
         */
        doParsing2(data, i, start, select, (year, arrRecruitmentRate, arrCompetitionRate) => {
            temp1.push({// chart.data에 i년도의 값들을 집어넣어라!
                year: String(year),
            ...
            if (temp1.length === end - start + 1) {
                temp1.sort((a, b) => { return a.year < b.year ? -1 : a.year > b.year ? 1 : 0 });
                temp2.sort((a, b) => { return a.year < b.year ? -1 : a.year > b.year ? 1 : 0 });
                callback();
            }
        });
    }
...
function doParsing2(data, year, minus, select, callback) {
    ...
    $.getJSON('../json/' + file, (jsonData) => {
        ...
        while (len <= jsonData.length) {
            if (len === jsonData.length) {
                callback(year, (data.arrRecruitmentRate[year - minus] / cnt).toFixed(1), (data.arrCompetitionRate[year - minus] / cnt).toFixed(1));
                break;
            }
            /**
             * select == 1 > 전체
            ...
            if (jsonData[len].recruitment_within_admission === 0 || jsonData[len].recruitment_over_admission === 0) { len++; continue; }

            if (select === 1) {
                data.arrRecruitmentRate[year - minus] += (jsonData[len].man_admitted_within_admission + jsonData[len].woman_admitted_within_admission) / jsonData[len].recruitment_within_admission * 100;
                data.arrCompetitionRate[year - minus] += jsonData[len].applied_within_admission / jsonData[len].recruitment_within_admission;
            } else if (select === 2) {
                data.arrRecruitmentRate[year - minus] += (jsonData[len].man_admitted_within_admission + jsonData[len].woman_admitted_within_admission) / jsonData[len].recruitment_within_admission * 100;
                data.arrCompetitionRate[year - minus] += jsonData[len].applied_within_admission / jsonData[len].recruitment_within_admission;
            } else {
                data.arrRecruitmentRate[year - minus] += (jsonData[len].man_admitted_over_admission + jsonData[len].woman_admitted_over_admission) / jsonData[len].recruitment_over_admission * 100;
                data.arrCompetitionRate[year - minus] += jsonData[len].applied_over_admission / jsonData[len].recruitment_over_admission;
            }
            cnt++;
            len++;
        }
...
function drawLineChart(arr) {
    var dataObject = { // 필요한 값 뽑아내는데 필요한 배열
    ...
    for (var i = 0; i < 10; i++) { // 10개년
        dataObject.arrRecruitmentRate.push(0.0);
        dataObject.arrCompetitionRate.push(0.0);
    }
    iter2(temp1, temp2, dataObject, 2010, 2019, select, () => { // JSON 파일 파싱 시작!
        ...
        var tooltipRecruitment = (select === 1) ? "전체 충원율" : (select === 2) ? "정원내 충원율" : "정원외 충원율"
        var tooltipCompetition = (select === 1) ? "전체 경쟁률" : (select === 2) ? "정원내 경쟁률" : "정원외 경쟁률"
        createAxisAndSeries2(chart1, "competition_rate", tooltipCompetition, false, "circle");
        createAxisAndSeries2(chart2, "recruitment_rate", tooltipRecruitment, false, "triangle");
...
function iter3(temp1, temp2, data, year, select, callback) {
    doParsing3(data, year, select, (recruitmentRateWithin, recruitmentRateOver) => {
        temp1.push(
...
function doParsing3(data, year, select, callback) {
    ...
    $.getJSON('../json/' + file, (jsonData) => {
        ...
        while (len <= jsonData.length) {
            if (len === jsonData.length) {
                callback((data.recruitmentRateWithin / cnt).toFixed(1), (data.recruitmentRateOver / cnt).toFixed(1));
                break;
            }
            if (jsonData[len].recruitment_within_admission === 0 || jsonData[len].recruitment_over_admission === 0) { len++; continue; }
            if (select === 1) {
                data.recruitmentRateWithin += (jsonData[len].man_admitted_within_admission + jsonData[len].woman_admitted_within_admission) / jsonData[len].recruitment_within_admission * 100;
                data.recruitmentRateOver += (jsonData[len].man_admitted_over_admission + jsonData[len].woman_admitted_over_admission) / jsonData[len].recruitment_over_admission * 100;
