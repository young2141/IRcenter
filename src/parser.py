...
file = xlrd.open_workbook('./2010년 지원자, 모집인원, 입학자.xls')
sheet = file.sheet_by_index(0)
nrows = sheet.nrows
data = []

for row_num in range(3,nrows-1):
    data.append({
        ...
        'total_applied':sheet.cell_value(row_num,11),
        'applied_within_admission':sheet.cell_value(row_num,12),
        'applied_over_admission':sheet.cell_value(row_num,13),
        'total_admitted':sheet.cell_value(row_num,15),
        'man_admitted_within_admission':sheet.cell_value(row_num,16),
        'woman_admitted_within_admission':sheet.cell_value(row_num,17),
        'man_admitted_over_admission':sheet.cell_value(row_num,18),
        'woman_admitted_over_admission':sheet.cell_value(row_num,19),
        'recruitment_rate':sheet.cell_value(row_num,20),
        'competition_rate':sheet.cell_value(row_num,21),
        })
'''
...
'''
with open('2010년 지원자, 모집인원, 입학자'+'.json','w',encoding='UTF-8-sig') as outfile:
   outfile.write(json.dumps(data,ensure_ascii=False))
