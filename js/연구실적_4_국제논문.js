        ...
        for (var j = 0; j < majors.length; j++) {
            ...
            if (sex == "(전체)") {
                ...
                if (json[i]["국제_SCI_남자"][majors[j]] != "" && json[i]["국제_SCI_남자"][majors[j]] != 0)
                    obj_temp["국제논문_SCI"] += json[i]["국제_SCI_남자"][majors[j]]
                if (json[i]["국제_SCI_여자"][majors[j]] != "" && json[i]["국제_SCI_여자"][majors[j]] != 0)
                    obj_temp["국제논문_SCI"] += json[i]["국제_SCI_여자"][majors[j]]

                if (json[i]["국제_기타_남자"][majors[j]] != "" && json[i]["국제_기타_남자"][majors[j]] != 0)
                    obj_temp["국제논문_기타"] += json[i]["국제_기타_남자"][majors[j]]
                if (json[i]["국제_기타_여자"][majors[j]] != "" && json[i]["국제_기타_여자"][majors[j]] != 0)
                    obj_temp["국제논문_기타"] += json[i]["국제_기타_여자"][majors[j]]
            }
            else {
                ...
                if (json[i]["국제_SCI_" + sex][majors[j]] != "" && json[i]["국제_SCI_" + sex][majors[j]] != 0)
                    obj_temp["국제논문_SCI"] += json[i]["국제_SCI_" + sex][majors[j]]

                if (json[i]["국제_기타_" + sex][majors[j]] != "" && json[i]["국제_기타_" + sex][majors[j]] != 0)
                    obj_temp["국제논문_기타"] += json[i]["국제_기타_" + sex][majors[j]]
            }
        }
            
        obj_temp["국제논문_SCI_1인당"] = (obj_temp["국제논문_SCI"] / prof_num).toFixed(4)
        obj_temp["국제논문_기타_1인당"] = (obj_temp["국제논문_기타"] / prof_num).toFixed(4)
    ...
    parse1(json1=> {
        ...
        parse2(json2 => {
            ...
            am4core.ready(function () {
                ...
                createSeries("국제논문_SCI" + _sort1, "SCI급/SCOPUS학술지")
                createSeries("국제논문_기타" + _sort1, "기타국제발간일반학술지")
