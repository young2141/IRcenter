    ...
    am4core.ready(function () {
        ...
        function P1CurvedcreateSeries(value, clr) {
            ...
            if (_divName == "divchart1")
                if(value_kr == "전체")
                    bullet.tooltipText = "[#000 font-size: 15px]{categoryX}학년도 " + value_kr + " 경쟁률은 [bold]{valueY}:1[\] 입니다."
                else
                    bullet.tooltipText = "[#000 font-size: 15px]{categoryX}학년도 " + value_kr + "계열의 경쟁률은 [bold]{valueY}:1[\] 입니다."
            else 
                if(value_kr == "전체")
                    bullet.tooltipText = "[#000 font-size: 15px]{categoryX}학년도 " + value_kr + " 충원율은 [bold]{valueY}%[\] 입니다."
                else
                    bullet.tooltipText = "[#000 font-size: 15px]{categoryX}학년도 " + value_kr + "계열의 충원율은 [bold]{valueY}%[\] 입니다."
